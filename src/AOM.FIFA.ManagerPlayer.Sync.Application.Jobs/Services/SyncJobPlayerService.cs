...
namespace AOM.FIFA.ManagerPlayer.Sync.Application.Jobs.Services
{
    public class SyncJobPlayerService : ISyncJobPlayerService
    {
        ...
        private FIFAManagerPlayerRequest MapToPlayerRequest(p.Player player)
        {
            ...
            playerRequest.AttackWorkRate = player.attack_work_rate ?? string.Empty;
            playerRequest.SourceClubId = player.club;
            playerRequest.CommonName = player.common_name ?? string.Empty;
            playerRequest.Defending = player.defending;
            playerRequest.DefenseWorkRate = player.defense_work_rate ?? string.Empty;
            playerRequest.Dribbling = player.dribbling;
            playerRequest.Foot = player.foot;
            playerRequest.Height = player.height;
            playerRequest.LastName = player.last_name ?? string.Empty;
